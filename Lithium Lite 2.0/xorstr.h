...
#define XORSTR_FORCE 
#else
#define XORSTR_FORCE __attribute__((always_)) 
#endif
...
namespace jm {
    ...
    namespace detail {
        ...
        XORSTR_FORCE constexpr std::size_t _buffer_size()
        {
            return ((Size / 16) + (Size % 16 != 0)) * 2;
        }

        template<std::uint32_t Seed>
        XORSTR_FORCE constexpr std::uint32_t key4() noexcept
        {
        ...
        XORSTR_FORCE constexpr std::uint64_t key8()
        {
        ...
        XORSTR_FORCE constexpr std::uint64_t
        ...
        XORSTR_FORCE std::uint64_t load_from_reg(std::uint64_t value) noexcept
        {
#if defined(__clang__) || defined(__GNUC__)
            asm("" : "=r"(value) : "0"(value) : );
#endif
    ...
    class xor_string<CharT, Size, std::integer_sequence<std::uint64_t, Keys...>, std::index_sequence<Indices...>> {
        ...
        constexpr static  std::uint64_t alignment = ((Size > 16) ? 32 : 16);
#else
        constexpr static  std::uint64_t alignment = 16;
#endif
    ...
    public:
        ...
        template<class L>
        XORSTR_FORCE xor_string(L l, std::integral_constant<std::size_t, Size>, std::index_sequence<Indices...>) noexcept
            : _storage{ JM_XORSTR_LOAD_FROM_REG(detail::uint64_v<detail::load_xored_str8<Size>(Keys, Indices, l())>::value)... }
        {}

        XORSTR_FORCE constexpr size_type size() const noexcept
        {
            return Size - 1;
        }

        XORSTR_FORCE void crypt() noexcept
        {
        ...
        XORSTR_FORCE const_pointer get() const noexcept
        {
            return reinterpret_cast<const_pointer>(_storage);
        }

        XORSTR_FORCE pointer get() noexcept
        {
            return reinterpret_cast<pointer>(_storage);
        }

        XORSTR_FORCE pointer crypt_get() noexcept
        {
            // crypt() function d by hand, because MSVC linker chokes when you have a lot of strings
